...
//    }
    ...
    public Person return_Staff(Person p) throws SQLException {
        ...
        try {
            connection = Singleton.getConnection();
            String query_Authentication = "SELECT p.idPerson, p.username, p.password, p.role, t.idType FROM ((staff stf inner join person p on stf.idPerson = p.idPerson) inner join type t on stf.idType = t.idType) where p.username=? and p.password=? and p.role=?";
